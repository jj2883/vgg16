...
vgg16_config = [64,64, 'Max', 128, 128, 'Max', 256, 256, 256, 'Max', 512, 512, 512, 'Max']
...
#        train_sampler = None
    ...
    train_loader = torch.utils.data.DataLoader(datasets.CIFAR10('../data', train=True, download=True, transform = transforms.Compose([transforms.RandomResizedCrop(224), transforms.RandomHorizontalFlip(), transforms.ColorJitter(), transforms.ToTensor(), transforms.normalize(mean = [0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])), batch_size = 256, shuffle = False, num_workers = args.workers, pin_memory = True)
